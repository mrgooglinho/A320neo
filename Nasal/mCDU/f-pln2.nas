...
var f_pln2 = {
	...
	init_f_pln : func {
		...
		for (var index = 0; getprop(altn_rte~"route/wp[" ~ index ~ "]/wp-id") != nil; index += 1) {
		
			var wp_id = getprop(altn_rte~"route/wp[" ~ index ~ "]/wp-id");
			
			var wp_alt = getprop(altn_rte~"route/wp[" ~ index ~ "]/altitude-ft");
		...
		for (var wp = 0; wp < max_wp; wp += 1) {
			...
			var wp_spd = getprop(altn_rte~"route/wp[" ~ wp ~ "]/ias-mach");
		...
		var dep = getprop(altn_rte~"depicao");
		
		var arr = getprop(altn_rte~"arricao");
	...
	cpy_to_stby : func {
		...
		for (var wp = 0; getprop(rm_route~ "route/wp[" ~ wp ~ "]/id") != nil; wp += 1) {
			...
			setprop(altn_rte~"route/wp[" ~ wp ~ "]/wp-id", getprop(rm_route~ "route/wp[" ~ wp ~ "]/id"));
			...
			if (alt != nil)
				setprop(altn_rte~"route/wp[" ~ wp ~ "]/altitude-ft", alt);
				
			if (spd != nil)
				setprop(altn_rte~"route/wp[" ~ wp ~ "]/ias-mach", spd);
